...
| --- | --- | --- | --- |
| Linux x86_64, CPython 3.12 | Soportado para la candidatura local | Suite CLI en contenedor Debian/Python 3.12 con Git real; wheel y sdist instalados sin monorepo; TAR repetido byte a byte; Unicode, espacios y CRLF | Gitleaks 8.x y Git deben estar en `PATH`; el operador debe verificar el artefacto y ejecutar `inspectra doctor` |
...
digest contractual. Esa matriz remota permanece separada del soporte local y
no se activará mientras `SEC-012`/`PROD-130` sigan bloqueadas.
