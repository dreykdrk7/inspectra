...
- `docs/future/active-dns-osint-frontend-ct-osint-review.md`
- `docs/future/active-dns-osint-ct-v0-functional-closeout.md`
- `docs/future/active-dns-v0-operational-guide.md`
- `docs/future/active-http-basic-header-review-design.md`
...
The Active DNS OSINT frontend product-flow decision is `ACTIVE_DNS_OSINT_07_FRONTEND_CT_OSINT_PRODUCT_FLOW_PASSED`. The React UI now exposes a separate `Active / DNS OSINT` panel that submits the exact CT-bounded backend contract, requires authorization/domain/public-OSINT confirmations, keeps passive DNS unavailable, selects the returned `202 JobRecord`, refreshes the job list, and renders CT OSINT output as redacted `osint_best_effort` public-source observed-name review indicators. The browser does not contact CT, passive DNS, provider APIs, DNS resolvers, archive/run-all, or `tools/runner/main.py`.
