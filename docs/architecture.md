...
The Passive Alpha owner metadata write-path runtime decision is `PASSIVE_ALPHA_RUNTIME_OWNER_METADATA_WRITE_PATH_ACCEPTED`. New uploads, file-based jobs, target-based jobs, and Active dry-run/header-probe jobs now write `owner_id` metadata. In `trusted_local_no_auth`, the owner is the default local/admin operator `local-admin`; legacy ownerless records remain readable for now. This slice does not implement login, password verification, sessions, cookies, multi-user runtime, owner-scoped reads, migrations, delete/retention runtime, frontend UI, billing, SaaS tenants, Nmap, new Active behavior, or new analyzers.
