...
3. A user starts file analysis with `POST /audits/pdf/{file_id}`, `POST /audits/image/{file_id}`, `POST /audits/manifest/{file_id}`, `POST /audits/archive/{file_id}`, `POST /audits/project-archive/{file_id}`, `POST /audits/django-config/{file_id}`, `POST /audits/docker-config/{file_id}`, `POST /audits/secrets-review/{file_id}`, or `POST /audits/node-package-config/{file_id}`. Web, domain, and subdomain inventory jobs are already created by `POST /audits/web/basic`, `POST /audits/domain/basic`, or `POST /audits/subdomains/basic` and store `target_url` or `target_domain` instead of `file_id`.
