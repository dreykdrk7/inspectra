...
servicios oficiales durante la aceptación expresamente autorizada de Eventora;
...
| PROD-007 | P2 | pendiente | Repositorios de solo lectura, tras modelo de amenaza |
...
| PROD-021 | P3 | pendiente | Asignación, comentarios y colaboración contextual |
| PROD-022 | P3 | pendiente | Métricas de tendencia y objetivos de riesgo |
...
| PROD-039 | P3 | pendiente | Identidad empresarial federada |
...
| PROD-071 | P3 | pendiente | Eventos firmados a integraciones |
| PROD-072 | P3 | pendiente | Métricas de adopción privadas |
...
| PROD-117 | P1 | completada | Aceptación real de Eventora con OSV/KEV oficiales y limpieza integral |
...
| PROD-237 | P3 | pendiente | Evaluador CVSS v4 validado con corpus oficial |
...
- **Actualización real 2026-09-06:** `PROD-117` consultó OSV y CISA KEV
  oficiales sobre el commit inmutable autorizado de Eventora. Descubrió y cerró
...
- **Pendientes y razón de prioridad:** `PROD-077` ya completó la revisión
  ...
  con el GO acotado de Eventora. La consolidación de `PROD-129` fue autorizada
  después y está en progreso sobre `release-prep/inspectra-0.3-beta`;
...
- **Actualización de aceptación Eventora (2026-09-06):** la revisión se repitió
...
- **Actualización de candidatura `PROD-116` (2026-09-06):** se construyeron
  ...
  `PROD-117` quedó preparada y posteriormente fue autorizada para `urlbreve`.
...
- **Tamaño:** L
- **Evidencia de validación al completarse:** pendiente; una revisión iniciada
  el 2026-09-09 no implementó ni validó el checkout seguro y no constituye
  evidencia de cierre.
...
- **Tamaño:** M
- **Evidencia de validación al completarse:** pendiente. La evidencia de
  retención y baja que figuraba aquí correspondía a `PROD-124`; no demuestra
  asignación, comentarios, menciones ni paginación de colaboración.
...
`urlbreve` produjo un `NO-GO` honesto por carecer de versiones exactas y el
commit inmutable `1368a0ca3d8f6b0ddefaf411ae809875f2767105` de Eventora cerró
...
`PROD-129`, cuya consolidación local y versión `0.3.0-beta.1` se autorizaron el
2026-09-10 y están en progreso; `PROD-130` depende además de CI remoto y de `SEC-012`.
...
- **Dependencias:** `PROD-129` y `SEC-012`; requiere autorización expresa para
  modificar `SEC-012` y para todo push/PR. No se dispone de ninguna de ellas.
- **Tamaño:** M
...
- **Evidencia de validación al completarse:** pendiente; `SEC-012` permanece
  bloqueada y no hubo push, PR, tag ni despliegue.
...
bloqueadas; `PROD-129` está en progreso tras su autorización local. Las tareas
...
| **PROD-237 — P3 — pendiente**<br>Evaluador CVSS v4 validado con corpus oficial | Seguridad necesita derivar v4 solo si el cálculo puede demostrarse, no aproximarse. Flujo: vector validado → evaluación oficial → banda/origen → detalle/informe. | `cvss.py`, normalizadores, UI/report y fixtures. | Algoritmo completo versionado, corpus FIRST y fronteras de redondeo; igualdad con resultados oficiales, score publicado intacto y `unknown` ante forma no soportada. | Una puntuación v4 aproximada altera prioridad y remediación. | PROD-097 y corpus oficial CVSS v4 revisado. | M | Pendiente; hoy v4 solo conserva un score válido publicado por la fuente. |
...
| **PROD-245 — P2 — completada**<br>Validador automático de coherencia entre backlogs | Producto y agentes necesitan una fuente de verdad consistente para elegir trabajo y no confundir evidencia histórica con estado actual. Flujo: ejecutar check local/CI → parsear resúmenes y fichas → comparar IDs/prioridad/estado → informar contradicciones sin editar. | Helper bajo `tools/`, fixtures, CI/documentación y ambos backlogs. | Detecta IDs duplicados o ausentes, prioridad/estado contradictorios y cambios en `SEC-012`, `PROD-129`, `PROD-130` o `PROD-167` mientras deban seguir bloqueadas. Salida accionable, determinista y sin modificar documentos; fixtures válidos y conflictivos. | Una contradicción puede ocultar trabajo pendiente o declarar cierre falso. | PROD-239 completada. | S | **2026-09-10:** helper stdlib read-only para resumen/fichas/filas detalladas, diagnósticos deterministas con línea y bloqueo explícito de `SEC-012`, `PROD-129`, `PROD-130` y `PROD-167`. El primer run real halló siete contradicciones; evidencia confirmó como completadas `PROD-063/124/243`, mantuvo pendientes `PROD-007/021/222` y movió la evidencia de identidad mal ubicada desde `PROD-021` a `PROD-124`. Integrado como `make check-backlogs` en validación local, CI, README y ciclo de `AGENTS.md`, sin cambiar acciones ancladas. Pasaron 10/10 fixtures, 14/14 dirigidas y 474/474 pruebas tools (7,20 s; 28.996 KiB RSS Docker), `compileall`, puerta real y diff-check, sin red ni mutación de los documentos por el helper. |
