...
| `INSPECTRA_SESSION_TTL_SECONDS` | backend | `3600` | Single-admin in-memory session TTL and session-cookie max age. Login/logout endpoints use it for the `inspectra_session` cookie, mutating cookie-auth routes require the session-bound `X-CSRF-Token`, and the frontend now fetches auth status, performs login/logout, keeps CSRF in memory only, and sends `X-CSRF-Token` on mutating requests. Rate limiting and lockout remain future work. |
