...
- Validates opt-in Active / DNS OSINT contract requests when `INSPECTRA_ACTIVE_DNS_OSINT_ENABLED=true`; accepted requests return controlled `not_executed` metadata with `[REDACTED_DOMAIN]`, zero external/CT/passive-DNS queries, no jobs, no storage, and public-source observed-name review-indicator wording reserved for future runtime.
...
Active DNS OSINT backend contract gate is documented in `docs/future/active-dns-osint-backend-contract-gate.md` with decision `ACTIVE_DNS_OSINT_02_BACKEND_CONTRACT_GATE_ACCEPTED`. The backend now exposes `POST /active/network/dns-osint` behind `INSPECTRA_ACTIVE_DNS_OSINT_ENABLED=false` by default. Enabled valid requests validate the exact `live_dns_osint` / `ct_subdomain_discovery_bounded` body, authorization confirmations, CT/passive-DNS source flags, one-domain policy, and `max_names` cap before returning `not_executed`. It creates no jobs, persists no storage, sends no CT/passive-DNS/HTTP/DNS requests, handles no provider credentials, and does not touch frontend, archive/run-all, or `tools/runner/main.py`.
...
| `INSPECTRA_ACTIVE_DNS_OSINT_ENABLED` | backend | `false` | Enables only the `POST /active/network/dns-osint` backend contract gate. Enabled valid requests validate one authorized domain, bounded `max_names`, CT opt-in, passive-DNS disabled state, and explicit public-OSINT confirmations, then return controlled `not_executed` metadata with zero external requests, no jobs, and no storage. |
