...
- Creates opt-in Active / DNS OSINT jobs when `INSPECTRA_ACTIVE_DNS_OSINT_ENABLED=true`; accepted requests use an injectable fakeable CT source adapter, persist owner-scoped `[REDACTED_DOMAIN]` jobs with `osint_best_effort` coverage, redacted observed-name counters/samples, zero real external/CT/passive-DNS/DNS/HTTP requests, and public-source observed-name review-indicator wording. The default source is disabled and no real CT source is configured yet.
...
| `INSPECTRA_ACTIVE_DNS_OSINT_ENABLED` | backend | `false` | Enables the bounded `POST /active/network/dns-osint` backend route. Enabled valid requests create owner-scoped redacted `active_dns_osint` jobs using an injectable fakeable CT source adapter; the default source is disabled and no real CT/HTTP/DNS/passive-DNS/provider requests are sent in the current phase. |
