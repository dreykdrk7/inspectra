...
77. Active DNS OSINT Microphase 08 reviews the frontend CT OSINT product flow from `ea56384`. The review confirms the panel, exact request contract, bounded `max_names`, required confirmations, passive DNS disabled state, no browser-side CT/provider/DNS source calls, redaction-first report/Raw JSON, `osint_best_effort` rendering, and no exhaustive-discovery, vulnerability-proof, target-safety, or public-scanning claims.
78. Active DNS OSINT Microphase 09 closes `active_dns_osint` CT v0 as a bounded self-hosted/local/private capability: disabled by default, CT source disabled by default, one authorized domain, exact CT-bounded contract, optional `crt.sh` source only through explicit configuration, owner-scoped `[REDACTED_DOMAIN]` jobs, redaction-first report/export/Raw JSON/frontend, `osint_best_effort` coverage, public-source observed-name review indicators, and manual validation required. It adds no passive DNS, provider import, DNS query, browser-side CT call, observed-name auto-scan, archive/run-all, `tools/runner/main.py`, Nmap, Docker, subprocess, release, tag, or push behavior.
79. Active DNS Operations Microphase 01 adds the Active DNS v0 operational guide as docs-only operator guidance. It covers disabled-by-default inventory and OSINT flags, the accepted `crt.sh` source configuration, one authorized domain, exact inventory/OSINT boundaries, separate AXFR confirmation, safe `example.com` request examples, redaction rules, controlled result states, and troubleshooting without changing runtime behavior or performing live requests.
80. Active HTTP Basic Header Review Microphase 01 freezes a docs-only design for a future `active_http_basic_header_review` capability: disabled by default, one authorized `http://` or `https://` URL, one bounded header request, no browser-side target request, no crawling, no auth, no custom headers, no request body, redaction-first reports/Raw JSON/frontend, and review-indicator wording with manual validation required. It adds no endpoint, flag, storage, UI, tests, runtime behavior, Docker, Nmap, network request, archive/run-all, `tools/runner/main.py`, release, tag, or push behavior.
...
Active DNS OSINT frontend CT product-flow review is documented in `docs/future/active-dns-osint-frontend-ct-osint-review.md` with decision `ACTIVE_DNS_OSINT_08_FRONTEND_CT_OSINT_REVIEW_PASSED`. It reviews `ea56384` and confirms the UI contract, browser boundary, passive-DNS disabled state, report/Raw JSON redaction, `osint_best_effort` wording, and absence of exhaustive-discovery, vulnerability-proof, target-safety, or public-scanning claims.

Active DNS OSINT CT v0 functional closeout is documented in `docs/future/active-dns-osint-ct-v0-functional-closeout.md` with decision `ACTIVE_DNS_OSINT_09_FUNCTIONAL_CLOSEOUT_ACCEPTED`. It freezes the capability as disabled-by-default, CT-source-disabled-by-default, one authorized domain, exact CT-bounded contract, optional explicitly configured `crt.sh`, owner-scoped redacted jobs, `osint_best_effort` public-source observed-name review indicators, and no passive DNS/provider import/browser CT calls/observed-name auto-scan/archive-run-all behavior.

Active DNS v0 operational usage is documented in `docs/future/active-dns-v0-operational-guide.md` with decision `ACTIVE_DNS_OPERATIONS_01_OPERATIONAL_GUIDE_ACCEPTED`. It gives trusted local/private/self-hosted operators one concise guide for choosing inventory versus OSINT, enabling the required flags, using AXFR only with separate confirmation, interpreting controlled states, and preserving redaction-first reports, exports, Raw JSON, and frontend output.

Active HTTP Basic Header Review design is documented in `docs/future/active-http-basic-header-review-design.md` with decision `ACTIVE_HTTP_BASIC_HEADER_REVIEW_01_DESIGN_ACCEPTED`. It proposes a future disabled-by-default `active_http_basic_header_review` capability for one authorized URL, one bounded header request, conservative redirect/header indicators, redaction-first result surfaces, and no runtime behavior in the design phase.
