...
- It does not turn Active / DNS inventory into provider DNS/API import, broad subdomain enumeration, CT/passive-DNS runtime, DNS CLI execution, raw DNS packet storage, public scanner behavior, or all-records-found reporting. AXFR is attempted only when the bounded Active DNS request is explicitly enabled and separately confirms `zone_transfer_authorized_confirmed: true`; refused or failed AXFR remains controlled best-effort/partial review-indicator output.
