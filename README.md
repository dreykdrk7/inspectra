...
- It does not turn Active / DNS inventory into complete-zone coverage, AXFR, provider DNS/API import, broad subdomain enumeration, CT/passive-DNS runtime, DNS CLI execution, raw DNS packet storage, public scanner behavior, or all-records-found reporting; v0 DNS inventory is bounded best-effort or partial review-indicator output only.
...
Active DNS Inventory design is documented in `docs/future/active-dns-inventory-design.md` with decision `ACTIVE_DNS_INVENTORY_01_DESIGN_FROZEN`. It defines a future opt-in `active_dns_inventory` contract for one explicit authorized domain, bounded standard DNS record types, controlled subdomain discovery, explicit `coverage_level`, future AXFR/provider-zone import phases, owner-scoped redaction, and DNS configuration review-indicator wording. This phase is docs-only and adds no DNS query runtime, sockets, shell commands, Nmap, Docker, HTTP, frontend runtime, archive/run-all, `tools/runner/main.py`, release, tag, or push state.
