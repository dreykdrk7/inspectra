...
| `INSPECTRA_SESSION_TTL_SECONDS` | backend | `3600` | Single-admin in-memory session TTL and session-cookie max age. Login/logout endpoints use it for the `inspectra_session` cookie, and mutating cookie-auth routes require the session-bound `X-CSRF-Token`; frontend login/CSRF wiring remains future work. |
