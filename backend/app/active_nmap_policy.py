...
class ActiveNmapTargetPolicyResult:
    normalized_targets: tuple[str, ...]

    @property
    def target_count(self) -> int:
        return len(self.normalized_targets)


class ActiveNmapTargetPolicyError(ValueError):
    def __init__(self, reason_code: str) -> None:
        self.reason_code = reason_code
        super().__init__(reason_code)


def validate_active_nmap_basic_targets(targets: object) -> ActiveNmapTargetPolicyResult:
    if not isinstance(targets, list) or not targets:
        raise ActiveNmapTargetPolicyError("targets_not_list")
    if len(targets) > ACTIVE_NMAP_BASIC_MAX_TARGETS:
        raise ActiveNmapTargetPolicyError("too_many_targets")

    normalized_targets = tuple(_normalize_single_target(target) for target in targets)
    if len(set(normalized_targets)) != len(normalized_targets):
        raise ActiveNmapTargetPolicyError("duplicate_targets")

    return ActiveNmapTargetPolicyResult(normalized_targets=normalized_targets)


def _normalize_single_target(target: object) -> str:
    if not isinstance(target, str):
        raise ActiveNmapTargetPolicyError("target_not_string")

    value = target.strip()
    if not value:
        raise ActiveNmapTargetPolicyError("target_empty")
    if value != target:
        raise ActiveNmapTargetPolicyError("target_ambiguous")
    if len(value) > ACTIVE_NMAP_BASIC_MAX_TARGET_LENGTH:
        raise ActiveNmapTargetPolicyError("target_too_long")
    if any(character.isspace() for character in value):
        raise ActiveNmapTargetPolicyError("target_list_in_string")
    if any(marker in value for marker in _DISALLOWED_TARGET_MARKERS):
        raise ActiveNmapTargetPolicyError("target_unsupported_syntax")
    if any(value.lower().startswith(marker) for marker in _DISALLOWED_SCHEME_MARKERS):
        raise ActiveNmapTargetPolicyError("target_url_not_allowed")
    if _looks_like_dash_range(value):
        raise ActiveNmapTargetPolicyError("target_range_not_allowed")
    if _looks_like_port_suffix(value):
        raise ActiveNmapTargetPolicyError("target_port_not_allowed")
...
def _normalize_ip_target(parsed_ip) -> str:
    if any(parsed_ip in blocked_network for blocked_network in _BLOCKED_IP_NETWORKS):
        raise ActiveNmapTargetPolicyError("target_special_purpose_blocked")
    if parsed_ip.version == 4 and (str(parsed_ip).endswith(".0") or str(parsed_ip).endswith(".255")):
        raise ActiveNmapTargetPolicyError("target_broadcast_or_network_like")
    ...
    if not any(parsed_ip in allowed_network for allowed_network in allowed_networks):
        raise ActiveNmapTargetPolicyError("target_not_local_private")
...
def _normalize_hostname_target(value: str) -> str:
    ...
    try:
        ...
        raise ActiveNmapTargetPolicyError("target_non_ascii") from exc
    ...
    if normalized in _BLOCKED_HOSTNAMES or any(fragment in normalized for fragment in _BLOCKED_HOSTNAME_FRAGMENTS):
        raise ActiveNmapTargetPolicyError("target_control_plane_blocked")
    if not _is_valid_hostname(normalized):
        raise ActiveNmapTargetPolicyError("target_invalid_hostname")
